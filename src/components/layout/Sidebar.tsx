...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-[300px] h-screen sticky top-0 shrink-0 bg-[#0b0e14] text-white overflow-y-auto">
      {/* Logo */}
        ...
        {nav.map(({ href, label, icon: Icon, color, bg }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                "flex items-center gap-3 rounded-xl px-3 py-3 text-[1.4rem] font-medium transition-all duration-150 group",
                active
            ...
            >
              <div className={cn("h-12 w-12 rounded-lg flex items-center justify-center shrink-0 transition-all", active ? "bg-indigo-500/10" : "bg-white/5 group-hover:bg-white/8")}>
                <Icon className={cn("h-6 w-6", active ? "text-indigo-400" : "text-slate-400 group-hover:text-slate-300")} />
              </div>
              <span className="flex-1">{label}</span>
              {active && <ChevronRight className="h-5 w-5 text-slate-400 shrink-0" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="px-3 pb-5 pt-3 border-t border-white/5 space-y-1 mt-3">
        <Link
          href="/settings"
          className={cn(
            "flex items-center gap-3 rounded-xl px-3 py-3 text-[1.4rem] font-medium transition-all duration-150",
            pathname === "/settings"
              ? "bg-white/10 text-white"
              : "text-slate-400 hover:bg-white/5 hover:text-slate-200"
          )}
        >
          <div className={cn("h-12 w-12 rounded-lg flex items-center justify-center shrink-0", pathname === "/settings" ? "bg-slate-500/10" : "bg-white/5")}>
            <Settings className={cn("h-6 w-6", pathname === "/settings" ? "text-slate-300" : "text-slate-400")} />
          </div>
          Settings
        </Link>

        <div className="flex items-center gap-3 rounded-xl px-3 py-3 text-[1.4rem] font-medium text-slate-400 hover:bg-white/5 hover:text-slate-200 transition-all duration-150 cursor-pointer">
        ...
        >
          <UserButton />
          <span className="text-[1.4rem] text-slate-400 font-medium">Account</span>
        </button>
        ...
        >
          ...
          <span className="text-[1.4rem] text-slate-400 group-hover:text-red-400 font-medium transition-colors">Sign out</span>
        </button>
      </div>
    </aside>
  );
