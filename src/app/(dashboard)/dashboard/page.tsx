...
function getRejectionRisk(score: number): { label: string; color: string; bg: string; advice: string } {
  if (score < 40) return { label: "Very High", color: "text-red-400", bg: "bg-red-500/10", advice: "Apply after reaching 60%" };
  if (score < 60) return { label: "High", color: "text-amber-400", bg: "bg-amber-500/10", advice: "Close critical gaps first" };
  if (score < 75) return { label: "Medium", color: "text-amber-400", bg: "bg-amber-500/10", advice: "Strengthen interview skills" };
  if (score < 88) return { label: "Low", color: "text-emerald-400", bg: "bg-emerald-500/10", advice: "Start applying now" };
  return { label: "Very Low", color: "text-emerald-400", bg: "bg-emerald-500/10", advice: "You're highly competitive" };
}

function getInterviewReadiness(avgScore: number | null, sessionCount: number): { label: string; color: string } {
  if (sessionCount === 0) return { label: "Not practiced", color: "text-slate-400" };
  if (!avgScore || avgScore < 45) return { label: "Needs work", color: "text-red-400" };
  if (avgScore < 65) return { label: "Getting there", color: "text-amber-400" };
  if (avgScore < 80) return { label: "Ready", color: "text-emerald-400" };
  ...
  return (
    <div className="space-y-7">
      ...
      {readinessScore !== null && rejectionRisk && (
        <div className={`rounded-2xl border p-6 flex items-start justify-between gap-6 flex-wrap ${readinessScore >= TARGET_READINESS ? "bg-emerald-500/10 border-emerald-500/20" : "bg-red-500/10 border-red-500/20"}`}>
          <div className="flex items-start gap-4">
            <div className={`h-11 w-11 rounded-xl flex items-center justify-center shrink-0 ${readinessScore >= TARGET_READINESS ? "bg-emerald-500/15" : "bg-red-500/15"}`}>
              <AlertTriangle className={`h-5 w-5 ${readinessScore >= TARGET_READINESS ? "text-emerald-400" : "text-red-400"}`} />
            </div>
            <div>
              <p className={`text-xs font-bold uppercase tracking-widest mb-1 ${readinessScore >= TARGET_READINESS ? "text-emerald-400" : "text-red-400"}`}>
                If you apply today
              </p>
        ...
        <div className="bg-[#1a1b23] rounded-2xl border border-red-500/20 shadow-sm p-6">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <Link href="/gaps" className="shrink-0">
              <button className="h-10 px-5 rounded-xl bg-red-500/10 hover:bg-red-500/15 text-red-400 font-semibold text-sm transition-colors flex items-center gap-2">
        ...
        <div className="bg-[#1a1b23] rounded-2xl border border-white/10 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="font-bold text-white">Day {dayOfJourney} of 90 · {progressPct}% complete</p>
            </div>
            <Link href="/roadmap">
