...
export default function OutreachPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-4xl font-bold text-white">Cold Outreach Generator</h1>
      ...
      <div className="bg-[#161820] rounded-2xl border border-white/10 shadow-sm p-6 space-y-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <div>
            <p className="font-bold text-white text-xl">Paste job description</p>
        ...
        <Textarea
          ...
          className="min-h-[160px] text-xl resize-none rounded-xl border-slate-200 focus:border-red-300"
          disabled={generating}
        />
        ...
        <Input
          ...
          className="rounded-xl border-slate-200 text-xl focus:border-red-300"
          disabled={generating}
        />

        {/* Tone selector */}
        <div className="space-y-2">
          <p className="text-base font-semibold text-slate-400 uppercase tracking-widest">Tone</p>
          <div className="flex gap-2">
            {TONE_OPTIONS.map(({ value, label }) => (
              <button
                ...
                disabled={generating}
                className={`px-4 py-1.5 rounded-full text-xl font-medium border transition-all ${
                  tone === value
        ...
        {error && (
          <div className="flex items-center gap-2 text-xl text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3">
      ...
      {result && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            <p className="text-base font-bold text-slate-500 uppercase tracking-widest mb-2">
              Subject line
            </p>
            <div className="flex items-center justify-between gap-4 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
              <p className="text-xl font-semibold text-slate-900 flex-1">{result.subject}</p>
            ...
            {result.emails.map((email) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xl font-bold text-slate-900 bg-slate-100 border border-slate-200 px-3 py-1 rounded-full">
                    {email.variant}
                  </span>
                  <CopyButton text={`Subject: ${result.subject}\n\n${email.content}`} />
                </div>
                <p className="text-2xl text-slate-700 leading-relaxed whitespace-pre-wrap">
                  {email.content}
                </p>
              </div>
            ))}
