    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <button className="w-full text-left px-6 py-4 flex items-center gap-4" onClick={() => setOpen(!open)}>
        <div className={`h-14 w-14 rounded-xl border flex flex-col items-center justify-center shrink-0 ${scoreBg}`}>
          <span className={`text-2xl font-black ${scoreColor}`}>{match.matchScore}</span>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            ...
            {match.redFlags && match.redFlags.length > 0 && (
              <span className="inline-flex items-center gap-1 text-sm font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full border border-red-200">
      ...
      {open && (
        <div className="px-6 pb-6 border-t border-slate-200 pt-5 space-y-6 bg-white rounded-b-2xl">
            ...
            <div className="flex-1 space-y-4">
              {match.improvementPlan && (
                <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                  <p className="text-sm font-bold text-indigo-600 uppercase tracking-widest mb-2">To reach 75%+</p>
                  <p className="text-base text-indigo-700 leading-relaxed">{match.improvementPlan}</p>
                </div>
              )}
              {/* Strengths */}
              <div>
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-2.5">Evidence of match</p>
                <div className="space-y-2">
                  {match.strengths.map((s, i) => (
                    <div key={i} className="flex items-start gap-2 text-base text-slate-700 bg-emerald-50 rounded-lg px-3 py-2 border border-emerald-100">
          ...
          {match.redFlags && match.redFlags.length > 0 && (
            <div className="rounded-xl border-2 border-red-200 bg-red-50 p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-base font-bold text-red-600 uppercase tracking-widest">Red flags detected</p>
              </div>
              <div className="space-y-2">
                {match.redFlags.map((flag, i) => (
                  <div key={i} className="flex items-start gap-2 text-base text-red-700">
          ...
          {/* Blocking gaps */}
          <div>
            <p className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-3">Blocking gaps — what&apos;s actually missing</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        <span className="font-bold text-slate-900 text-base">{gap.label}</span>
                        <span className={`text-sm font-bold px-2 py-0.5 rounded-full border ${SEVERITY_STYLE[gap.severity]}`}>
                          {gap.severity}
                        </span>
                      ...
                      {gap.signalFound && (
                        <div className="flex items-start gap-1.5 bg-white border border-slate-200 rounded-lg px-3 py-1.5 mb-2">
                          ...
                          <p className="text-sm text-slate-500 italic">Found in resume: {gap.signalFound}</p>
                        </div>
                      )}
                      ...
                      <div className="flex items-start gap-1.5">
                        ...
                        <p className="text-base text-slate-700">{gap.action}</p>
                      </div>
                    </div>
                  </div>
                </div>
...
export default function JobMatchPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-4xl font-bold text-white">Job Match Score</h1>
      ...
      <div className="bg-[#161820] rounded-2xl border border-white/10 shadow-sm p-6 space-y-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">Copy the full JD — the more detail, the more accurate your score</p>
          </div>
        </div>
        ...
        <Textarea
          ...
          className="min-h-[180px] text-base resize-none rounded-xl border-slate-200 focus:border-indigo-300"
          disabled={analyzing}
        />
      ...
      {result && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-bold text-white text-xl">Your match result</h2>
            <Button variant="ghost" size="sm" className="gap-1.5 text-slate-400 text-base" onClick={() => setResult(null)}>
          ...
          {result.matchScore < 70 && (
            <div className="bg-indigo-500/10 rounded-2xl border border-indigo-500/20 p-5 flex items-start gap-4">
              ...
              <div className="flex-1">
                <p className="font-bold text-indigo-300 text-base">Close these gaps with your roadmap</p>
                <p className="text-xl text-indigo-400 mt-0.5">Your 90-day plan already includes tasks that address these gaps. Check your current week.</p>
              </div>
              <a href="/roadmap">
                <Button size="sm" className="gap-1.5 shrink-0 bg-indigo-600 hover:bg-indigo-500 text-white text-base">
          ...
          {result.matchScore >= 70 && (
            <div className="bg-emerald-500/10 rounded-2xl border border-emerald-500/20 p-5 flex items-start gap-4">
              ...
              <div className="flex-1">
                <p className="font-bold text-emerald-400 text-base">You&apos;re a strong candidate — apply now</p>
                <p className="text-xl text-emerald-400 mt-0.5">A 70%+ match means you should apply. Most engineers wait too long. Go for it.</p>
              </div>
            </div>
          )}
        </div>
      ...
      {!loadingHistory && history.filter(m => m.id !== result?.id).length > 0 && (
        <div className="space-y-3">
          <h2 className="font-bold text-white text-xl">Previous analyses</h2>
      ...
      {!loadingHistory && history.length === 0 && !result && (
        <div className="text-center py-12 text-slate-400">
          ...
          <p className="text-xl mt-1">Paste a job description above to see how well you match</p>
        </div>
      )}
    </div>
  );
